...
void omm_opt_init_main_menu() {
    ...
    for (s32 i = 0; i != menuMain.numOpts; ++i) {
        ...
        // Custom Touch Controls sub-menu
        if (menuMain.opts[i].type == OPT_SUBMENU && menuMain.opts[i].nextMenu == &menuTouchControls) {
            head->type = OPT_SUBMENU;
            head->label = gOmmOptTouchControls.label;
            head->nextMenu = (struct SubMenu *) gOmmOptTouchControls.subMenu;
            head++;
            continue;
        }
#endif
